...
export function calculateBusinessScore(business: any, issues?: any) {
  ...
  // If we have a Lighthouse score, use it as the primary metric
  if (lighthouseScore > 0) {
    // Invert the Lighthouse score to get the Shit Score
    // Lighthouse: 90 (excellent) -> Shit Score: 10 (excellent)
    // Lighthouse: 30 (poor) -> Shit Score: 70 (poor)
    const score = Math.max(0, Math.min(100, 100 - lighthouseScore));
    
    console.log(`Using inverted Lighthouse score: ${score}`);
    business.score = score;
    
    return score;
  }
  
  // Fallback to issue-based scoring if no Lighthouse score is available
  console.log('No Lighthouse score available, using issue-based scoring');
